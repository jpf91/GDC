 ...
 * Please not that each Fiber has a Context, but each thread also has got a Context
 * which describes the threads stack and state. If you call Fiber fib; fib.call
 * the first time in a thread you switch from ThreadContext into the Fibers Context.
 ...
 *     //We have now switch to the stack of a different Context!
 *     copy {b} into {stack pointer}
 *     //We now have to adjust the stack pointer to point to 'Real Stack End' so we can pop the FP registers
 ...
 *     jump to {return Address}
 *
 * So the question now is which registers need to be saved? This depends on the specific architecture ABI of course,
 * but here are some general guidelines:
 * - If a register is callee-save (if the callee modifies the register it must saved and restored by the callee)
 *   it needs to be saved/restored in switchContext
 * - If a register is caller-save it needn't be saved/restored. (Calling fiber_switchContext is a function call
 *   and the compiler therefore already must save these registers before calling fiber_switchContext)
 ...
 * - The link register usually needn't be saved/restored (but sometimes it must be cleared - see below for details)
 * - The frame pointer register - if it exists - is usually callee-save
 * - All current implementations do not save control registers
 *
 * What happens on the first switch into a Fiber? We never saved a state for this fiber before, but the initial state is
 * prepared in the initStack routine. (This routine will also be called when a Fiber is being resetted). initStack
 * must produce exactly the same stack layout as the part of fiber_switchContext which saves the registers. Pay special
 * attention to set the stack pointer correctly if you use the GC optimization mentioned before. the return Address
 * saved in initStack must be the address of fiber_entrypoint.
 *
 * There's now a small but important difference between the first context switch into a fiber and further context switches.
 * On the first switch, Fiber.call is used and the returnAddress in fiber_switchContext will point to fiber_entrypoint.
 * The important thing here is that this jump is a _function call_, we call fiber_entrypoint by jumping before it's function
 * prologue. On later calls, the user used yield() in a function, and therefore the return address points into a user
 * function, after the yield call. So here the jump in fiber_switchContext is a _function return_, not a function call!
 *
 * The most important result of this is that on entering a function, i.e. fiber_entrypoint, we would have to provide a
 * return address / set the link register once fiber_entrypoint returns. Now fiber_entrypoint does never return and
 * therefore the actual value of the return address / link register is never read/used and therefore doesn't matter.
 * When fiber_switchContext performs a _function return_ the value in the link register doesn't matter either.
 * However, the link register will still be saved to the stack in fiber_entrypoint and some exception handling /
 * stack unwinding code might read it from this stack location and crash. The exact solution depends on your architecture,
 * but see the ARM implementation for a way to deal with this issue.
 *
 * The ARM implementation is meant to be used as a kind of documented example implementation. Look there for a concrete
 * example.
 * 
